...
// CNIManager handles CNI configuration and setup
...
func (c *CNIManager) SetupCilium() error {
...
func (c *CNIManager) removeFlannel() error {
...
func (c *CNIManager) InstallCNIPlugins(version string) error {
	...
	if err := utils.RunCommand("which", "curl"); err != nil {
...
func (c *CNIManager) CreateBridgeConfig() error {
	...
	if _, err := os.Stat(configPath); err == nil {
		logrus.Info("Bridge CNI configuration already exists, skipping creation")
		return nil
	}
	...
	if err := utils.RunSystemCommand("cp", tempBridgeFile.Name(), configPath); err != nil {
		return fmt.Errorf("failed to install bridge config file: %w", err)
	}
...
func (c *CNIManager) ValidateCNI() error {
