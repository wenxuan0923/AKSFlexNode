...
remove_arc_agent() {
    ...
    if command -v azcmagent &> /dev/null; then
        # Try to clear agent configuration first
        log_info "Clearing Azure Arc agent configuration..."
        azcmagent config clear || {
            log_warning "Failed to clear Arc agent configuration - continuing with removal"
        }

        # Check if there's an uninstall script
        if [[ -f /opt/azcmagent/bin/azcmagent_uninstall.sh ]]; then
            log_info "Running Azure Arc agent uninstall script..."
            /opt/azcmagent/bin/azcmagent_uninstall.sh || {
                log_warning "Arc agent uninstall script failed - removing files manually"
            }
        else
            log_info "No uninstall script found - removing files manually"
        fi

        # Remove Arc agent files manually
        log_info "Removing Azure Arc agent files..."
        rm -rf /opt/azcmagent || true
        rm -rf /var/opt/azcmagent || true
        rm -rf /etc/opt/azcmagent || true
        rm -f /usr/bin/azcmagent || true
        rm -f /etc/systemd/system/himdsd.service || true

        # Remove himds user and group if they exist
        if id himds &>/dev/null; then
            userdel himds 2>/dev/null || true
        fi
        if getent group himds &>/dev/null; then
            groupdel himds 2>/dev/null || true
        fi

        # Reload systemd daemon
        systemctl daemon-reload || true

        log_success "Azure Arc agent removed"
    else
        log_info "Azure Arc agent not found"
    fi
}
...
show_completion_message() {
    ...
    echo "✅ Azure Arc connection and cluster resources"
    echo "✅ Azure Arc agent (azcmagent)"
    echo ""
