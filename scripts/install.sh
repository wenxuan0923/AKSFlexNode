...
check_azure_cli_auth() {
    ...
    if ! command -v az &> /dev/null; then
        ...
        log_info ""
        log_info "AKS Flex Node can use either:"
        log_info "  1. Service Principal authentication (recommended for production)"
        log_info "  2. Azure CLI authentication (for development/testing)"
        log_info ""
        log_info "If you want to use Azure CLI authentication:"
        log_info "  - Install Azure CLI: curl -sL https://aka.ms/InstallAzureCLIDeb | sudo bash"
        log_info "  - Run 'az login' to authenticate"
        log_info "  - Then re-run this installer"
        log_info ""
        echo -n "Do you want to continue without Azure CLI? (service principal auth only) [y/N]: "
        read -r response
        case "$response" in
            [yY]|[yY][eE][sS])
                log_info "Continuing without Azure CLI. Make sure to configure service principal authentication."
                return 0
                ;;
            *)
                log_info "Installation cancelled. Please install Azure CLI and authenticate first."
                exit 0
                ;;
        esac
    else
